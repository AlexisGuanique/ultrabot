...
from app.ultrabot.ultra_bot import (
    execute_ultra_bot,
    stop_ultra_bot,
    execute_ultra_bot_repetidas,
...
def setup_ui(logged_in_user, on_login_success):
    ...
    def clear_db():
        ...
        if confirm:  # Si el usuario hace clic en "Sí"
            try:
                ...
                messagebox.showinfo(
                    "Cookies Limpiadas",
                    "Todas las cookies fueron eliminadas exitosamente.",
                )
            except Exception as e:
                messagebox.showerror(
                    "Error al Limpiar",
                    f"Se produjo un error al limpiar la base de datos:\n{e}",
                )
        else:
            # Mensaje opcional
            messagebox.showinfo("Acción cancelada", "No se han eliminado las Cookies.")
    ...
    def handle_ultra_bot():
        ...
        if not (token_data and token_data.get("is_valid")):
            messagebox.showerror(
                "Usuario expirado",
                "Usuario expirado, por favor contacte con los desarrolladores.",
            )
            logout()
            root.destroy()
    ...
    def handle_stop_ultra_bot_repetidas():
        ...
        messagebox.showinfo(
            "Ultra Bot Repetidas",
            "Ultra Bot de cuentas repetidas detenido correctamente.",
        )
    ...
    def save_repetidas_settings_ui():
        ...
        try:
            ...
            success = save_repetidas_settings(
            ...
            if success:
                ...
                messagebox.showerror(
                    "Error",
                    "No se pudo guardar la configuración de repetidas en la base de datos.",
                )
    ...
    logout_button = ctk.CTkButton(
        ...
        corner_radius=10,
        width=160,
        height=40,  # Botón más pequeño en ancho
    ...
    signature_label.place(
        relx=0.0, rely=1.0, anchor="sw", x=10, y=-5
    ...
    ultra_email_entry = ctk.CTkEntry(
        ultra_config_frame, width=200, placeholder_text="Ej: usuario@correo.com"
    )
    ...
    ultra_pass_frame = ctk.CTkFrame(
        ultra_config_frame, fg_color="transparent", width=200, height=40
    )
    ultra_pass_frame.pack(pady=(0, 10))

    ultra_pass_entry = ctk.CTkEntry(
        ultra_pass_frame, width=200, placeholder_text="••••••••", show="*"
    )
    ...
    # 👉 Input: Ruta del exe de Ultra
    ultra_exe_label = ctk.CTkLabel(
        ultra_config_frame,
        text="Ruta del exe de Ultra:",
        text_color="black",
        font=("Arial", 12, "bold"),
    )
    ultra_exe_label.pack(pady=(0, 2), anchor="w")

    ultra_exe_entry = ctk.CTkEntry(
        ultra_config_frame,
        width=200,
        placeholder_text=r"C:\Users\Administrator\AppData\Local\Programs\Ultra\Ultra.exe",
    )
    ultra_exe_entry.pack(pady=(0, 10))
    ...
    ultra_exe_path = get_ultra_exe_path()
    if ultra_exe_path:
        ultra_exe_entry.insert(0, ultra_exe_path)
    ...
    def save_ultra_credentials_ui():
        ...
        exe_path = ultra_exe_entry.get()

        if not email or not password or not exe_path:
            messagebox.showerror("Error", "Por favor completa todos los campos.")
            return

        if not os.path.exists(exe_path):
            messagebox.showerror("Error", "La ruta del exe de Ultra no existe.")
            return

        if not exe_path.lower().endswith(".exe"):
            messagebox.showerror("Error", "La ruta debe apuntar a un archivo .exe.")
            return

        save_ultra_credentials(email, password)
        set_ultra_exe_path(exe_path)
        messagebox.showinfo(
            "Guardado",
            "✅ Credenciales y ruta del exe de Ultra guardadas correctamente.",
        )
    ...
    repetidas_accounts_entry = ctk.CTkEntry(
        repetidas_config_frame, width=200, placeholder_text="Ej: 5"
    )
    ...
    repetidas_count_entry = ctk.CTkEntry(
        repetidas_config_frame, width=200, placeholder_text="Ej: 3"
    )
    ...
    repetidas_time_entry = ctk.CTkEntry(
        repetidas_config_frame, width=200, placeholder_text="Ej: 120"
    )
    ...
    if repetidas_settings:
        repetidas_accounts_entry.insert(
            0, str(repetidas_settings["accounts_to_repeat"])
        )
