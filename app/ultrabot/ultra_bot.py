...
from app.database.database import (
    ...
    get_password_by_id,
    get_bot_settings,
    get_ultra_credentials,
    get_user_agent_by_id,
    clear_database,
    fetch_accounts_from_server,
    save_cookies_to_db,
...
def wait_for_login_interface(max_attempts=3, wait_time=15):
    ...
    for attempt in range(max_attempts):
        print(
            f"🔍 Verificando interfaz de login (intento {attempt + 1}/{max_attempts})..."
        )
        ...
        if attempt < max_attempts - 1:  # No esperar en el último intento
            print(
                f"⏳ Interfaz de login no detectada. Esperando {wait_time} segundos..."
            )
...
def login_with_ultra_credentials():
    ...
    if not password_field_clicked:
        print(
            "ℹ️ Campo de contraseña no detectado por imagen, usando navegación por teclado"
        )
    ...
    if current_content:
        ...
        print(
            f"   Campo contiene: {'*' * len(current_content)} (asteriscos por seguridad)"
        )
    else:
...
def find_and_click_input(cookie_id_override=None):
    ...
    cookie_id_to_use = (
        cookie_id_override if cookie_id_override is not None else last_cookie_id
    )
    ...
    if not cookie_text:
        ...
        messagebox.showinfo(
            "Ejecución finalizada", "Bot detenido por falta de cookies."
        )
        stop_ultra_bot()
    ...
    if not user_agent:
        ...
        messagebox.showerror(
            "Falta User Agent",
            f"No se encontró user agent para el ID {cookie_id_to_use}",
        )
        stop_ultra_bot()
...
def close_codigo(espanol=False):
    ...
    images = (
        ...
        if not espanol
        else ["app/ultrabot/images/codigoVerificacion/codigoVerificacionEspanol.png"]
    )

    if any(find_image(image) for image in images):
        print(
            "🚀 Código de verificación detectado. Iniciando proceso de verificación..."
        )
        run_checker()
        ...
        if find_image("app/ultrabot/images/accionesVentana/enterCode.png"):
            ...
            print(
                "⚠️ No se encontró el campo para ingresar el código, pero se intentará pegar el código igualmente."
            )
        ...
        if find_image("app/ultrabot/images/accionesVentana/submitBoton.png"):
            ...
            print(
                "⚠️ No se encontró el botón de submit, pero se intentará hacer clic igualmente."
            )
...
def click_image_multiple(
    image_paths, description="", fallback_coords=None, confidence=0.7
):
...
def click_image_with_fallback(
    ...
    list_image_found = any(
        find_image(image, confidence=confidence) for image in image_list
    )
...
def click_europa_boton():
    return click_image_multiple(
        [
            "app/ultrabot/images/accionesVentana/ventanaGrisEuropa.png",
        ],
        description="boron de europa",
        fallback_coords="249 x 197",
        confidence=0.9,
    )
...
def click_europa_boton2():
    return click_image_multiple(
        ["app/ultrabot/images/accionesVentana/ventanaGrisEuropa3.png"],
        description="boton de eutopa espanol",
        fallback_coords="269 x 218",
        confidence=0.9,
    )


def click_ultra_logo():
    # Commented out: Original logic using image recognition to find and click Ultra logo
    # return click_image_multiple(
    #     [
    ...
    #     ],
    #     description="Logo de ultra",
    #     fallback_coords="171 x 749",
    # )

    # New logic: Launch Ultra exe directly from configured path
    exe_path = get_ultra_exe_path()
    if not exe_path or not os.path.exists(exe_path):
        print("❌ Ruta del exe de Ultra no válida o no existe.")
        return False

    try:
        subprocess.run([exe_path])
        time.sleep(15)  # Wait for app to start, same as original
        print("✅ Ultra exe lanzado correctamente.")
        return True
    except Exception as e:
        print(f"❌ Error al lanzar el exe de Ultra: {e}")
        return False
...
def click_add_account():
    return click_image_multiple(
        [
            "app/ultrabot/images/agregarCuenta/agregarCuenta.png",
            "app/ultrabot/images/agregarCuenta/agregarCuenta3.png",
            "app/ultrabot/images/agregarCuenta/agregarCuentaIngles.png",
            "app/ultrabot/images/agregarCuenta/agregarCuentaIngles2.png",
        ],
        description="botón de agregar cuenta",
        fallback_coords="1243 x 167",
    )
...
def click_panel_dropDown():
    return click_image_multiple(
        [
            "app/ultrabot/images/panelDesplegableDown/panelDesplegableDown.png",
            "app/ultrabot/images/panelDesplegableDown/panelDesplegableDown2.png",
            "app/ultrabot/images/panelDesplegableDown/panelDesplegableDown3.png",
        ],
        description="panel desplegable",
        fallback_coords="585 x 92",
    )
...
def click_add_cookie():
    return click_image_multiple(
        [
            "app/ultrabot/images/ingresarCookie/ingresarCookies.png",
            "app/ultrabot/images/ingresarCookie/ingresarCookies2.png",
            "app/ultrabot/images/ingresarCookie/ingresarCookies3.png",
        ],
        description="botón de agregar cookie",
        fallback_coords="751 x 109",
    )
...
def click_ok_button():
    return click_image_multiple(
        [
            "app/ultrabot/images/botonOk/botonOk4.png",
...
def click_menu_me():
    return click_image_multiple(
        [
            "app/ultrabot/images/menuDesplegable/menuDesplegableMe.png",
            "app/ultrabot/images/menuDesplegable/menuDesplegableMe2.png",
            "app/ultrabot/images/menuDesplegable/menuDesplegableYo.png",
            "app/ultrabot/images/menuDesplegable/menuDesplegableYo2.png",
        ],
        description="menú desplegable Me",
        fallback_coords="978 x 164",
    )
...
def click_sign_out():
    return click_image_multiple(
        [
            "app/ultrabot/images/singout/signOut.png",
            "app/ultrabot/images/singout/signOut2.png",
            "app/ultrabot/images/singout/signOutEspanol.png",
            "app/ultrabot/images/singout/signOutEspanol2.png",
        ],
        description="botón de cerrar sesión",
        fallback_coords="787 x 573",
    )
...
def click_location():
    return click_image_multiple(
        [
            "app/ultrabot/images/location/locationImage.png",
            "app/ultrabot/images/location/locationImageEspanol.png",
        ],
        description="Pantalla de location",
        fallback_coords="702 x 108",
    )
...
def click_login_whit_email():
    ...
    additional_image = (
        "app/ultrabot/images/loginPassword/loginPasswordEnglishIncomplete3.png"
    )
...
def click_login_whit_email_incomplete():
    return click_image_multiple(
        ...
        description="Botón incompleto de iniciar sesión con Email",
        fallback_coords="302 x 409",
        confidence=0.9,
    )
...
def click_close_boton():
    return click_image_multiple(
        [
            "app/ultrabot/images/loginPassword/loginExit2.png",
            "app/ultrabot/images/loginPassword/loginExit.png",
        ],
        description="botón de X de detener el loguin",
        fallback_coords="836 x 429",
    )
...
def click_sing_in():
    return click_image_multiple(
        [
            "app/ultrabot/images/loginPassword/loginPasswordBotonEnglish.png",
            "app/ultrabot/images/loginPassword/loginPasswordBotonEnglish2.png",
            "app/ultrabot/images/loginPassword/loginPasswordBotonEspanol2.png",
            "app/ultrabot/images/loginPassword/loginPasswordBotonEspanol.png",
        ],
        description="botón de iniciar sesión",
        fallback_coords="659 x 448",
    )
...
def click_remember_me():
    return click_image_multiple(
        [
            "app/ultrabot/images/loginPassword/logoutEspanol.png",
            "app/ultrabot/images/loginPassword/logoutEspanol2.png",
            "app/ultrabot/images/loginPassword/logoutEnglish.png",
        ],
        description="botón de remember me",
        fallback_coords="680 x 370",
    )
...
def click_options_forget_account():
    return click_image_multiple(
        [
            "app/ultrabot/images/loginPassword/loginOptions2.png",
            "app/ultrabot/images/loginPassword/loginOptions.png",
        ],
        description="botón de opcion de olvidar cuenta",
        fallback_coords="842 x 329",
    )
...
def click_forget_account():
    return click_image_multiple(
        [
            "app/ultrabot/images/loginPassword/forgetAccount2.png",
            "app/ultrabot/images/loginPassword/forgetAccount.png",
            "app/ultrabot/images/loginPassword/forgetAccountEspanol.png",
            "app/ultrabot/images/loginPassword/forgetAccountEspanol2.png",
        ],
        description="botón de iniciar sesión",
        fallback_coords="773 x 371",
    )
...
def click_minimize_window():
    return click_image_multiple(
        [
            "app/ultrabot/images/accionesVentana/minimizarVentana.png",
            "app/ultrabot/images/accionesVentana/minimizarVentana2.png",
        ],
        description="botón de minimizar ventana",
        fallback_coords="166 x 45",
    )


def click_close_window():
    return click_image_multiple(
        ["app/ultrabot/images/accionesVentana/cerrarVentana.png"],
        description="botón de cerrar ventana",
        fallback_coords="183 x 45",
    )
...
def click_refresh():
    return click_image_multiple(
        [
            "app/ultrabot/images/accionesVentana/recargarPestana.png",
            "app/ultrabot/images/accionesVentana/recargarPestana2.png",
            "app/ultrabot/images/accionesVentana/recargarPestana3.png",
        ],
        description="botón de recargar ventana",
        fallback_coords="702 x 108",
    )
...
def click_start_all_tabs():
    return click_image_multiple(
        ["app/ultrabot/images/accionesVentana/activaDesactivaPestanas.png"],
        description="botón de iniciar todas las tabs",
        fallback_coords="1290 x 100",
    )
...
def click_stop_all_tabs():
    return click_image_multiple(
        ["app/ultrabot/images/accionesVentana/activaDesactivaPestanas.png"],
        description="botón de recargar ventana",
        fallback_coords="1179 x 100",
    )


def click_acept_actionTabs():
    return click_image_multiple(
        ["app/ultrabot/images/accionesVentana/aceptaActivaDesactivaPestana.png"],
        description="boton para aceptar arrancar las tabs o detenerlas",
        fallback_coords="976 x 445",
    )


def click_acept_stop_actionTabs():
    return click_image_multiple(
        ["app/ultrabot/images/accionesVentana/aceptarStopTabs.png"],
        description="boton para aceptar o detenerlas",
        fallback_coords="974 x 212",
    )
    ...
    def run(self):
        ...
        while self.running:
            if iteration_count >= MAX_ITERATIONS:
                ...
                print(
                    f"📊 Dividiendo el tiempo de espera en 4 partes de {TIEMPO_ESPERA // 4} segundos cada una..."
                )
                ...
                for parte in range(3):
                    ...
                    if processes_killed:
                        ...
                        print(
                            "⚠️ Algunos procesos de Ultra no pudieron ser terminados, continuando..."
                        )
                    ...
                    print(
                        f"⏱️ Parte {parte_numero}/4: Esperando {tiempo_por_parte} segundos..."
                    )
                    time.sleep(tiempo_por_parte)
                    print(f"✅ Parte {parte_numero}/4 completada.")

                print(
                    f"✅ Tiempo de espera completo ({TIEMPO_ESPERA} segundos) finalizado."
                )
                ...
                print(
                    "🔄 Proceso finalizado, reiniciando el contador de iteraciones..."
                )
                ...
                if not processes_killed:
                    print(
                        "⚠️ No se pudieron detener algunos procesos de Ultra, continuando..."
                    )
                else:
                ...
                for cache_attempt in range(max_cache_attempts):
                    print(
                        f"🗑️ Intento de eliminación de cache {cache_attempt + 1}/{max_cache_attempts}..."
                    )
                    cache_deleted = handle_delete_ultra_folder(
                    ...
                    if cache_deleted:
                        ...
                        print(
                            f"❌ Fallo en intento {cache_attempt + 1}, reintentando..."
                        )
                        if (
                            cache_attempt < max_cache_attempts - 1
                ...
                if not cache_deleted:
                    print(
                        "❌ Error crítico: No se pudo eliminar la cache después de 3 intentos"
                    )
                    messagebox.showerror(
                        "Error",
                ...
                for restart_attempt in range(max_restart_attempts):
                    print(
                        f"🔄 Reiniciando Ultra (intento {restart_attempt + 1}/{max_restart_attempts})..."
                    )
                    ...
                    if click_ultra_logo():
                        ...
                        print(
                            "🔐 Esperando a que la interfaz de login esté disponible..."
                        )
                        if wait_for_login_interface(max_attempts=3, wait_time=15):
                            ...
                            print(
                                f"❌ No se pudo detectar la interfaz de login después de varios intentos (intento {restart_attempt + 1})"
                            )
                            if (
                                restart_attempt < max_restart_attempts - 1
                            ):  # No cerrar en el último intento
                                print("🔄 Cerrando ventana y reintentando...")
                                # 🖱️ Cerrar ventana nuevamente
                                click_coordinates(1339, 10)
                                time.sleep(
                                    5
                                )  # Tiempo adicional para que Ultra se cierre
                                ...
                                if not cache_deleted:
                                    print(
                                        "⚠️ No se pudo eliminar la cache en el reintento, continuando..."
                                    )
                    else:
                        print(
                            f"❌ No se pudo hacer click en el logo de Ultra (intento {restart_attempt + 1})"
                        )
                        if (
                            restart_attempt < max_restart_attempts - 1
                        ):  # No cerrar en el último intento
                ...
                if not login_successful:
                    print(
                        "❌ No se pudo completar el login después de todos los reintentos"
                    )
                    messagebox.showerror(
                        "Error",
                ...
                if not accounts:
                    print(
                        "❌ No se pudieron obtener cuentas del servidor. Deteniendo el bot."
                    )
                    messagebox.showerror(
                        "Error",
                        "No se pudieron obtener cuentas del servidor. Verifica tu conexión y credenciales.",
                ...
                # Guardar las cuentas obtenidas en la base de datos local
                print(
                    f"💾 Guardando {len(accounts)} cuentas en la base de datos local..."
                )
                save_cookies_to_db(accounts)
            ...
            if not find_and_click_input():
                print(
                    f"❌ Cookie con ID {last_cookie_id} inválida o rechazada. Saltando a la siguiente..."
                )
...
class UltraBotRepetidasThread(threading.Thread):
    ...
    def run(self):
        ...
        if config:
            ...
            print(
                "⚠️ No se encontró configuración de repetidas. Usando valores por defecto."
            )
        ...
        print(
            f"   - Tiempo de espera: {TIEMPO_ESPERA} segundos ({TIEMPO_ESPERA // 60} minutos)"
        )
        ...
        while self.running:
            ...
            for account_index in range(total_accounts):
                ...
                for repetition in range(REPETITIONS_COUNT):
                    ...
                    print(
                        f"   🔁 Repetición {repetition + 1}/{REPETITIONS_COUNT} de la cuenta {current_cookie_id}"
                    )
                    ...
                    if not find_and_click_input(cookie_id_override=current_cookie_id):
                        print(
                            f"   ❌ Cookie con ID {current_cookie_id} inválida o rechazada en repetición {repetition + 1}."
                        )
            ...
            print(
                f"⏳ Esperando {TIEMPO_ESPERA} segundos ({TIEMPO_ESPERA // 60} minutos) antes de continuar..."
            )
